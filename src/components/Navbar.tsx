          ...
          <div className="flex items-center space-x-4 pl-4 border-l border-[var(--panel-border)]">
            ...
            <div className="relative group pb-2 -mb-2">
              <div className="flex items-center space-x-2 hover:bg-[var(--panel-bg)] px-3 py-1.5 rounded-full transition-all cursor-pointer">
                <img src={session.user.image || `https://api.dicebear.com/7.x/avataaars/svg?seed=${session.user.username || 'Anonymous'}`} alt="avatar" className="w-8 h-8 rounded-full border border-blue-500" />
                <span className="text-[var(--text-muted)] font-bold">{session.user.username || 'Anonymous'} ▾</span>
              </div>
