...
/**
 * The Titan Engine (MVAR-15) - Corrected Calibration
 */
export async function calculateUserRating(userId: string) {
  ...
  // --- Pillar 1: Intensity Vector (Dampened) ---
  const attemptPowers = await Promise.all(attempts.map(async (a) => {
    ...
    const intensity = a.wpm * Math.pow(acc, 4) * (perc / 100)
    const diffMultiplier = 1 + Math.log(diffIndex)
    return intensity * diffMultiplier
  }))

  const sortedPowers = attemptPowers.sort((a, b) => b - a).slice(0, 25) // Focus on top 25
  let skillIndex = 0
  sortedPowers.forEach((p, i) => {
    skillIndex += p * Math.pow(0.75, i) // Sharper decay
  })

  // --- Pillar 2: Saturation (Volume) ---
  const vs = (20 * Math.log(1 + uniqueChallenges)) + 
             (10 * Math.sqrt(nHard)) + 
             (15 * Math.pow(nSuper, 0.5)) + 300 // +300 Prestige Floor

  // --- Pillar 3: Mechanical ---
  const vm = (topWpm * 0.25) + (avgWpm * 0.05) + (50 * consistency) + 300 // +300 Prestige Floor

  // --- Pillar 4: Momentum ---
  const vt = Math.sqrt(totalTimeSeconds / 3600) * 3 + (recentAvgWpm - avgWpm) * 0.1

  // --- Final Aggregation (Tuned for 3500 GM Peak) ---
